...
export const ClientPurchaseOrders: CollectionConfig = {
  slug: 'client-purchase-orders',
  labels: { singular: 'Client PO', plural: 'Client POs' },
  admin: {
    useAsTitle: 'poNumber',
    defaultColumns: ['poNumber', 'clientName', 'project', 'poDate', 'status'],
    group: 'Operations',
  },
  access: {
      ...
      async ({ data, operation, req }) => {
        if (operation === 'create' && !data.poNumber) {
          ...
          const existing = await req.payload.find({
            collection: 'client-purchase-orders',
            where: { poNumber: { like: `CPO-${year}-` } },
            sort: '-poNumber',
            limit: 1,
          })
          let next = 1
          ...
          data.poNumber = `CPO-${year}-${String(next).padStart(4, '0')}`
        }
        return data
      },
    ],
  ...
  fields: [
    {
      ...
      unique: true,
      admin: {
        description: 'Auto-generated on create (CPO-YYYY-####). Editable -- e.g. to record the client\'s own PO number.',
      },
    },
