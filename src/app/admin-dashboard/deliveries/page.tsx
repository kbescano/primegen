...
export default async function DeliveriesPage({
  ...
  // 4. Fetch Linked Itineraries
  const itinerariesRes = orderIds.length > 0 
    ? await payload.find({
  ...
  itinerariesRes.docs.forEach((itin: any) => {
    itineraryByOrderId[String(itin.sourceOrderId)] = itin;
  });

  // 5. Fetch Supplier POs
  const posRes = orderIds.length > 0
    ? await payload.find({
  ...
  const enrichedOrders = orders.map((o: any) => {
    const hasItinerary = !!itineraryByOrderId[o.id];
    const itin = itineraryByOrderId[o.id];
    const address = o.shippingAddress || o.deliveryAddress || o.address || (typeof o.client === 'object' ? o.client?.address : '') || 'No address provided';
    ...
    if (o.targetDeliveryDate) {
      ...
      targetDateStr = tDate.toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })
    ...
    const orderPOs = posRes.docs.filter((po: any) => {
      const sourceId = typeof po.sourceOrderId === 'object' ? po.sourceOrderId?.id : po.sourceOrderId;
  ...
  return <DeliveriesClient enrichedOrders={enrichedOrders} filterStatus={filterStatus} />
}