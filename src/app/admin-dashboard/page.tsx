...
const STEP_ORDER = ['quotation', 'confirmation', 'supplierPO', 'fulfilled', 'delivery', 'closed'] as const
...
function computeStepState(
  ...
  const completedSteps = {
    ...
    fulfilled: pos.length > 0 && pos.every((po: any) => po.status === 'fulfilled'),
...
export default async function QuotationInboxPage({
  ...
  searchParams: Promise<{ status?: string; granularity?: string; periodValue?: string; page?: string }>
}) {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-xs text-gray-500 font-medium max-w-[600px]">
            ...
            quotes are always sent by your team directly, never automatically. Showing {totalDocs}{' '}
            total.
          </p>
        </div>
      </div>
      ...
      {docs.length === 0 ? (
        <div className="border border-dashed border-gray-200 py-12 text-center rounded">
          <p className="text-xs text-gray-400 font-medium">
            No quotation requests{activeStatus ? ` with status "${STATUS_LABELS[activeStatus]}"` : ''} in
            this period.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-5">
          {docs.map((q: any) => {
            ...
            const { currentStep } = computeStepState(linkedQuotation, orderByQuotationId, posByOrderId)
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 bg-gray-50 border border-gray-100 rounded px-3.5 py-2.5 mb-4 text-[11px]">
                  <span className="font-bold text-[#01172f]/70">
                    Stage: <span className="text-[#149911] font-bold ml-1">{stageLabel}</span>
                  </span>
                  <Link
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                    ...
                    {Array.isArray(q.items) && q.items.length > 0 ? (
                      <div className="flex flex-col gap-2">
                        {q.items.map((item: any, i: number) => {
                          ...
                          const matUnit = typeof material === 'object' ? material?.unit : ''
                          return (
                      ...
                      <p className="text-[11px] text-gray-400 italic">No items listed</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {q.message ? (
                      ...
                      <p className="text-[11px] text-gray-400 italic">No message attached</p>
                    )}
                  </div>
                </div>
