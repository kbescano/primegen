...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [materialsRes, categoriesRes] = await Promise.all([
    payload.find({ collection: 'materials', limit: 1000, depth: 0 }),
    payload.find({ collection: 'categories', limit: 100, depth: 0 }),
  ])

  const staticPages: MetadataRoute.Sitemap = [
    { url: siteUrl, changeFrequency: 'weekly', priority: 1 },
    { url: `${siteUrl}/materials`, changeFrequency: 'daily', priority: 0.9 },
  ...
  const materialPages: MetadataRoute.Sitemap = materialsRes.docs.map((m: any) => ({
