...
const CATEGORIES = [
  {
    label: "Steel & Rebar",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 mb-2 sm:mb-3 text-[#fdfffc]">
...
export default function CinematicVideoHero({ slides: rawSlides }: { slides: HeroSlide[] }) {
  ...
  return (
    <section className="relative w-full h-[95vh] min-h-[750px] sm:min-h-[800px] overflow-hidden bg-[#01172f]">
      ...
      <div className="absolute inset-0 z-[3] max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 h-full flex flex-col justify-center pt-20 sm:pt-24 pb-[300px] sm:pb-[200px] lg:pb-32 pointer-events-none">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center flex-1">
          
          {/* Left Column: Dynamic Typography & CTAs */}
          <div className="relative w-full max-w-xl h-[420px] sm:h-[340px] flex items-center">
            {slides.map((slide, index) => (
              <div 
                ...
                `}
              >
                <p className="text-[11px] sm:text-[12px] md:text-[14px] font-bold uppercase tracking-[0.2em] text-[#149911] mb-2 sm:mb-3 md:mb-5">
                  {slide.label}
                </p>
                
                <h1 className="text-[36px] sm:text-[44px] md:text-[56px] lg:text-[64px] font-black text-[#fdfffc] leading-[1.05] tracking-tight mb-4 sm:mb-6">
                  {slide.title}
                </h1>
                ...
                <div className="flex flex-col sm:flex-row w-full sm:w-auto items-stretch sm:items-center gap-3 sm:gap-4">
                  <Link 
                    href="/contact"
                    className="w-full sm:w-auto text-center px-6 sm:px-8 py-3.5 bg-[#149911] text-white text-[13px] md:text-[14px] font-bold tracking-wide rounded-lg hover:bg-[#10750e] transition-colors duration-300 shadow-[0_0_20px_rgba(20,153,17,0.3)]"
                  >
                    Contact us
                  </Link>
                  <Link 
                    href={slide.href || '#'}
                    className="w-full sm:w-auto text-center px-6 sm:px-8 py-3.5 bg-transparent border border-[#fdfffc]/30 text-[#fdfffc] text-[13px] md:text-[14px] font-bold tracking-wide rounded-lg hover:bg-[#fdfffc]/10 transition-colors duration-300"
                  >
                    {slide.cta}
                  </Link>
                </div>
      ...
      <div className="absolute bottom-6 sm:bottom-12 md:bottom-8 left-0 right-0 w-full px-4 sm:px-6 lg:px-12 max-w-[1400px] mx-auto z-[4] pointer-events-none">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3 md:gap-4 pointer-events-auto">
          {CATEGORIES.map((cat, idx) => (
            <Link 
              key={idx}
              href={`/products#${cat.label.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}
              className="group flex flex-col items-start justify-end h-24 sm:h-28 md:h-32 p-3 sm:p-4 md:p-5 bg-[#fdfffc]/5 backdrop-blur-md border border-[#fdfffc]/10 rounded-2xl hover:bg-[#fdfffc]/10 hover:border-[#fdfffc]/30 transition-all duration-300"
            >
